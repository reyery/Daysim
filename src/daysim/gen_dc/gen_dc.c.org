...
void usage( char* command ) {
	...
	fprintf(stderr,"\t-dir calculate indirect-direct daylight coefficients only\n ");
	fprintf(stderr,"\t-dir calculate indirect-direct daylight coefficients only\n ");
...
int main( int argc, char **argv )
{
	...
	if( simple_blinds_model == 1 ) // if the simple blind model is used the daylight
		NumBlindSettings=1;   // coefficient files for blinds up and down are identical.

	//old dynamic blind model
	if(!AdvancedBlindModel && NumBlindSettings ==2)	{
		for( i= 0 ; i<NumBlindSettings ; i++) {
			if( strcmp(shading_rad_file[i],"") ) {
				sprintf(keyword,"%s",shading_rad_file[i]);
				strcpy(shading_rad_file[i],keyword);
			} else{
				fprintf(stderr,"ERROR gen_dc: no radiance input files given\n");
				exit(1);
			}
		}
	}
	...
	if(AdvancedBlindModel) // read in new, flexible blind model
		                   //##################################
	{
		for( i= 0 ; i <= NumBlindSettings ; i++ ) {
			if(ExtendedOutput)
				printf("daylight coefficients for: %s\n",BlindGroupName[i]);

			for (j=0 ; j<NumberOfSettingsInBlindgroup[i] ; j++) {
				if( task & CalculateDiffuse ) {  /* Calculate Diffuse Daylight Coefficients */
					if(ExtendedOutput)
						fprintf(stderr,"calculate diffuse daylight coefficients for variant %s...\n",BlindGroupName[i] );

					//write output files
					sprintf( octree, "%s/%s.setting_%d.dif.oct", tmp_directory, BlindGroupName[i],j );
					sprintf( dcFile, "%s/%s.setting_%d.dif.dc", tmp_directory, BlindGroupName[i],j );
					
					strcpy( tmpSensorFile, sensor_file );
					callOconv( ExtendedOutput, bin_dir, skyPatch, material_file, geometry_file,BlindGroupGeometryInRadiance[j][i], tmpSensorFile, octree );
					callRtraceDC( ExtendedOutput, bin_dir, "", octree, tmpSensorFile, dcFile, &diffuseOptions );

				}

				if( task & CalculateDirect ) {  /* Calculate Direct Daylight Coefficients */
					if(ExtendedOutput)
						fprintf(stderr, "calculate %d direct daylight coefficients for variant %s...\n", number_direct_coefficients, BlindGroupName[i] );
					sprintf( octree, "%s/%s.setting_%d.dir.oct", tmp_directory, BlindGroupName[i],j );
					sprintf( dcFile, "%s/%s.setting_%d.dir.dc", tmp_directory, BlindGroupName[i],j );
					strcpy( tmpSensorFile, sensor_file );
					callOconv( ExtendedOutput, bin_dir, direct_radiance_file, material_file,
							   geometry_file, BlindGroupGeometryInRadiance[j][i], tmpSensorFile, octree );
				...
				if( task & MergeFiles ) {       /* merge lines of files */
					if(ExtendedOutput)
						fprintf(stderr, "merge direct and diffuse daylight coefficients for variant %s blind setting %d...\n",BlindGroupName[i],j );

					sprintf( dcFile_dif, "%s/%s.setting_%d.dif.dc", tmp_directory, BlindGroupName[i],j );
					sprintf( dcFile_dir, "%s/%s.setting_%d.dir.dc", tmp_directory, BlindGroupName[i],j );

					mergeFiles( BlindGroupDiffDC_File[j][i], dcFile_dir, dcFile_dif );
				}

				if( (task & SubstractDirectDirect) && dds_file_format) {  /* Substract Direct Daylight Coefficients */
					if(ExtendedOutput)
						fprintf(stderr, "substract direct contribution from direct-indirect daylight coefficients for variant %s blind setting %d...\n",BlindGroupName[i],j );

					sprintf( octree, "%s/%s.setting_%d.dir.oct", tmp_directory, BlindGroupName[i],j );
					sprintf( dcFile, "%s/%s.setting_%d.dir.ab0.dc", tmp_directory, BlindGroupName[i] ,j);
					...
					//substract ab0
					Substract_ab0(BlindGroupDiffDC_File[j][i], dcFile );
				}

				if( (task & AddDirectDirect) && dds_file_format) {  /* Add DirectDirect component */
					if(ExtendedOutput)
						fprintf(stderr, "add direct-direct daylight coefficients for variant %s blind setting %d...\n",BlindGroupName[i],j );


					calculate_sky_patchesDDS(2305,direct_radiance_file_2305); // 2305 sun positions for dir DC
					sprintf( octree, "%s/%s.setting_%d.dir.oct", tmp_directory, BlindGroupName[i] ,j);
					sprintf( dcFile, "%s/%s.setting_%d.dir-dir.ab0.dc", tmp_directory, BlindGroupName[i],j );
					...
					USE_RTRACE_DC_2305=0;
					//add 2305 direct-direct coefficients

					Add_ab0(BlindGroupDiffDC_File[j][i], dcFile );
				}
			} //end for (j=0 ; j<NumberOfSettingsInBlindgroup[i] ; j++)
			//if(ExtendedOutput)
			fprintf(stderr, "Blind Group %d (%s) completed.\n\n\n",i,BlindGroupName[i]);
		}// end for( i= 0 ; i < NumBlindSettings ; i++ )
	} else { //old shading model
		for( i= 0 ; i < NumBlindSettings ; i++ ) {
			if(ExtendedOutput)
